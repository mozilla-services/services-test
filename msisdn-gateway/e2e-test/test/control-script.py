...
def run_msisdn_cli(client, cli_command, gateway_number, test_number):
    ...
    worker.sendline("cd ./msisdn-cli; virtualenv .venv; . .venv/bin/activate;"
                    " python ./setup.py develop")
    worker.sendline(cli_command)
    print("Executing command: " + cli_command)

    # retrieve the verification message
    worker.expect("Please\senter\sthe\scode\sthat\syou\
    swill\sget\sby\sSMS\sfrom\s")
    message_log = client.messages.list(
        gateway_number,
        test_number,
    ...
    # extract the code
    match = re.search(
        "^Your\sverification\scode\sis:\s(\d\d\d\d\d\d)",
...
def main():
    ...
    client = TwilioRestClient(
        account_auth["account_sid"], account_auth["auth_token"])
    clean_up_message_logs(client)
    command = "msisdn-cli -H " + host + " -c " + \
              numbers["mcc"] + " -n " + numbers["number"]
    run_msisdn_cli(
        client, command, args["host_number"], numbers["number"])
